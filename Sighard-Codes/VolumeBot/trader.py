...
class Trader:
    ...
    def can_sell(self, wallet, amount,x):
        sell_function = self._swap_tokens_for_eth(wallet, amount,x)
        gas_limit = self.estimate_gas(sell_function, wallet)
        gas_price = self.web3.eth.gas_price
        tx_fee = self._calc_tx_fee(gas_limit, gas_price)
        bnb_balance = self.get_bnb_balance(wallet)
        return bnb_balance - (tx_fee *6) > 0
    ...
    def sell(self, wallet, private_key, amount,slippage):
        try:
            ...
            sell_function = self._swap_tokens_for_eth(wallet, amount,slippage)
            ...
            logger.error(f"[ERROR] while [SELL]: {e} {slippage}")
