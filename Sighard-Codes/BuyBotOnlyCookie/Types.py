...
class Token:
    def __init__(self, WebThree : Web3, tokenAddress,routerAddress, chainID, chainName,dexName,dexLink,symbol) -> None:
        self.dexName = dexName
        self.WebThree = WebThree
        self.router_address = routerAddress
        self.chainID = chainID
        self.chainName = chainName
        self.dexLink = dexLink
        self.creating = [tokenAddress,chainID]
        
        #Router
        self.router_contract= self.WebThree.eth.contract(address=self.router_address, abi=json.load(open('router_abi.json')))
        ...
        #Pair
        wbnbAddress=Web3.toChecksumAddress(str(wbnb_address))
        ...
        self.pairContract = self.WebThree.eth.contract(address=str(self.pairAddress), abi=json.load(open('pair_abi.json')))
        self.fromBlock = self.WebThree.eth.blockNumber
        self.position = (wbnb_address == self.pairContract.functions.token0().call())
...
class FairLaunch:
    ...
    def getText(self, address, wbnb_amount, usd_amount,timestamp,txHash):
        ...
        text += "\n <b><a href= 'https://{}.com/address/{}'>👤 Buyer Position</a> New </b>".format( self.chainName,address)
        # text += "\n <b>🤑 Got:</b> A lot Of {} \n <b>💵 Spent:</b> {} {} (${})".format( self.symbol, wbnb_amount,self.chainSymbol, usd_amount)
        text += "\n <b>Presale:</b> {} \n <b>💵 Spent:</b> {} {} (${})".format( address, wbnb_amount,self.chainSymbol, usd_amount)
        text += f"\n <b>⏳ Time:</b> {timestampToHumanReadble(timestamp)}"
        text += f"\n<a href='https://{self.chainName}.com/tx/{txHash}'>👁‍🗨 TX</a>"
        return text
