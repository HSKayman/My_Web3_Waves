...
class ApiManager():
                ...
                for element in readFile:
                    if readFile[str(element)]['chainID'] == 56:
                        if int(readFile[str(element)]['isWhat']) > 0:
                            self.APIS[str(element)] = BSC(readFile[str(element)]['API_key'], readFile[str(
                                element)]['tokenAddress'], readFile[str(element)]['isWhat'], readFile[str(element)]['routerOrPair'])
                        else:
                            self.APIS[str(element)] = BSC(readFile[str(element)]['API_key'], readFile[str(
                                element)]['tokenAddress'], readFile[str(element)]['isWhat'])
                    elif readFile[str(element)]['chainID'] == 1:
                        if int(readFile[str(element)]['isWhat']) > 0:
                            self.APIS[str(element)] = ETH(readFile[str(element)]['API_key'], readFile[str(element)]['tokenAddress'], readFile[str(
                                element)]['isWhat'], readFile[str(element)]['alchemy'], readFile[str(element)]['routerOrPair'])
                        else:
                            self.APIS[str(element)] = ETH(readFile[str(element)]['API_key'], readFile[str(
                                element)]['tokenAddress'], readFile[str(element)]['isWhat'], readFile[str(element)]['alchemy'])
    ...
    def writeApi(self):
        ...
        for element in self.APIS:   
            writeFile[str(element)] =  {
                'chainID': self.APIS[element].chainID,
        ...
        with open('ApiLog.json', 'w') as f:
            json.dump(writeFile, f)

        # writeFile = defaultdict(list)
        # for element in self.APIS:
        #     writeFile[str(element)] = {
        #         'chainID': self.APIS[element].chainID,
        #         'API_key': self.APIS[element].API_key,
    ...
    def addChat(self, chatId, contract, api_key, chainID, isWhat, routerOrPair=None, alchemy=None):
        if chainID == 56:
            if (int(isWhat) == 3):
                self.APIS[str(chatId)] = BSC(api_key.replace(
                    ' ', ''), contract.replace(' ', ''), int(isWhat))
            elif int(isWhat) > 0:
                self.APIS[str(chatId)] = BSC(api_key.replace(' ', ''), contract.replace(
                    ' ', ''), int(isWhat), routerOrPair.replace(' ', ''))
            else:
                self.APIS[str(chatId)] = BSC(api_key.replace(
                    ' ', ''), contract.replace(' ', ''), int(isWhat))
        elif chainID == 1:
            if (int(isWhat) == 3):
                self.APIS[str(chatId)] = BSC(api_key.replace(' ', ''), contract.replace(
                    ' ', ''), int(isWhat), alchemy.replace(' ', ''))
            elif int(isWhat) > 0:
                self.APIS[str(chatId)] = ETH(api_key.replace(' ', ''), contract.replace(
                    ' ', ''), int(isWhat), alchemy.replace(' ', ''), routerOrPair.replace(' ', ''))
            else:
                self.APIS[str(chatId)] = ETH(api_key.replace(' ', ''), contract.replace(
                    ' ', ''), int(isWhat), alchemy.replace(' ', ''))
        self.writeApi()
    ...
    def removeToken(self, chatId, contract):#Fix it
        if str(chatId) in self.APIS.items():
            if self.APIS[str(chatId)].tokenAddress == contract:
                self.APIS.pop(str(chatId))
                self.writeApi()
                self.readApi()
                return True
        return False
