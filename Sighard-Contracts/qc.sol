...
contract LotteryStaking is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    constructor() VRFConsumerBaseV2(vrfCoordinator){
        ...
        uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(stakedToken), address(uniswapV2Router.WETH())));
...
//=======Staking Process=======//
    ...
    function deposit(uint256 _power) public nonReentrant{
        ...
        if(currentLottery.holderTicket[msg.sender]==0){ 
            uint256 ticket=uint256(keccak256(abi.encodePacked(msg.sender,block.number,block.timestamp)))%900_000 + 100_000;
    ...
    function withdraw(uint256 _stakeId) public nonReentrant{
        ...
        stakedToken.transferFrom(poolContract,address(msg.sender), _pendingReward);
...
//=======Lottery=======//
    ...
    function getPrice(address _user,uint256 _power) public view returns(bool,uint256,uint256,uint256,uint256,uint256){
        uint256 _price = 0;
        ...
        return (isDiscount,_price, _realPrice,getBNBPrice(_price*1e15),getBNBPrice(_realPrice*1e15),getBNBPrice(_price*1e15)*_power);
    }
    ...
    function getBNBPrice(uint256 amountIn) public view returns(uint256){
        ...
        if(address(uniswapV2Pair.token0()) == address(uniswapV2Router.WETH()))
            ...
            (frokInFrokPair, bnbInFrokPair, ) = uniswapV2Pair.getReserves();
            
        uint256 aForkWorthOfBNB = (frokInFrokPair * amountIn * (10**18)) / bnbInFrokPair;
