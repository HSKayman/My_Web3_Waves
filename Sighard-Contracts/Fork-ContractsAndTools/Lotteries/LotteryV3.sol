...
contract GoldPayLottery is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    uint8 public totalFeePercentage;
    uint8 public treasuryShare;
    uint8 public burnShare;
    ...
    uint256 public perEpoch; //For test
    ...
    event ShareChanged(uint8 _treasuryShare,uint8 _burnShare,uint8 _marketingShare);
    event FeePercentageChanged(uint8 _totalFeePercentage);
    event MarketingWalletChanged(address _marketingWallet);
    event TreasuryWalletChanged(address _treasuryWallet);
    ...
    function _pickWinner() internal {
        ...
        if(currentLottery.ticketOwner[winnerTicket]!= address(0)){
            ...
            balances[currentLottery.lotteryWinner]+=(currentLottery.totalRewards * multiplier[0])/10_000;
    ...
    function swap() internal{
        uint256 contractTokenBalance = accumulatedFees;
        ...
        if( canSwap && !swapping && swapEnabled) {
            ...
            RewardToken.safeTransfer(marketingWallet, marketingTokens);
            RewardToken.safeTransfer(treasuryWallet, treasuryTokens);
    ...
    function buyTicket(uint256[] memory tickets) public nonReentrant{
        ...
        if(totalFeePercentage>0){
            uint256 fee = (_ticketPrice * totalFeePercentage) / 100; 
            accumulatedFees+=fee;
    ...
    function changeTreasuryWallet(address _treasuryWallet) external onlyOwner {
        ...
        require(_treasuryWallet != address(0), "Treasury wallet is the zero address");
    ...
    function changedPerEpoch (uint256 _perEpoch) external onlyOwner {
        require(_perEpoch > 0, "Per epoch must be greater than 0");
        perEpoch = _perEpoch;
        emit PerEpochChanged(perEpoch);
    }
    ...
    function changeFeePercentage(uint8 _feePercentage) external onlyOwner {
        require(_feePercentage <= 30, "Fee percentage must be less than 25");
        totalFeePercentage = _feePercentage;
        emit FeePercentageChanged(totalFeePercentage);
    }
    ...
    function setShare(uint8 _burnShare, uint8 _marketingShare, uint8 _treasuryShare) external onlyOwner {
        require(_burnShare + _marketingShare + _treasuryShare == 100, "Shares must add up to 100");
        burnShare = _burnShare;
        marketingShare = _marketingShare;
        treasuryShare = _treasuryShare;
        emit ShareChanged(burnShare, marketingShare, treasuryShare);
    }
