...
contract ALEXStaking is Ownable, ReentrancyGuard {
    ...
    constructor(PoolInfo memory arg1) {   
        ...
        require(decimalsRewardToken < 40, "Should be inferio to 30");

        poolInfo[0].PRECISION_FACTOR = uint256(10**(40 - decimalsRewardToken));
    ...
    function emergencyWithdraw(uint256 _poolId) external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            if(pool.withdrawLockPeriod > 0) {
                uint fee=0;
                if(fees.emergencyWithdrawalFee>0){
                    fee = ( amountToTransfer * fees.emergencyWithdrawalFee ) / 10000;
    ...
    function addPool(PoolInfo memory arg1) external onlyOwner {
        ...
        poolInfo[_poolId].PRECISION_FACTOR = uint256(10**( 40 - decimalsRewardToken));
    ...
    function updatePoolLimitPerUser(uint256 _poolId, uint256 _poolLimitPerUser) external onlyOwner {
        PoolInfo storage pool = poolInfo[_poolId];
