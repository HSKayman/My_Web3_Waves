...
abstract contract randomNumberGenerater {
    function rand() external virtual returns (uint256);
}

abstract contract Game is Ownable, Pausable, Multicall {
    ...
    randomNumberGenerater public RNG;

    constructor(address _rng) {
        RNG = randomNumberGenerater(_rng);
    }
...
contract RollGame is Game {
    ...
    constructor(address _rng) Game(_rng) {}
    ...
    function wager(uint8 cap1, uint8 cap2, address token, uint256 tokenAmount) external payable whenNotPaused {
        ...
        fulfillRandomWords(bet.randomNumber);
    }

    function fulfillRandomWords(uint256 randomNumber) internal {
        uint8 cap1 = diceBets[randomNumber];
        uint8 cap2 = diceBets2[randomNumber];
        Bet storage bet = bets[randomNumber];
        uint256 rolled = randomNumber % 101;
        bool wins;
