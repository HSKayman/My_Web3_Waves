    ...
    contract LunanasStaking is Ownable, ReentrancyGuard {
        ...
        uint8 public refereeRate;
        uint256 public minimumStakedTokenForReferee;

        PoolInfo[] public poolInfo;
        mapping(address => StakeInfo[]) public stakeInfo;
        mapping(bytes32 => address) public refereeAddress;
        mapping(address => bytes32) public addressReferee;
        ...
        event ChangedFee(uint256 indexed poolId, uint256 depositFee, uint256 withdrawFee, uint256 emergencyWithdrawFee);
        event AddPool(uint256 indexed poolId, uint256 interest, uint256 lockPeriod, uint256 emergencyWithdrawFee, uint256 depositFee, uint256 withdrawFee);
        event GenerateReferral(address indexed user, bytes32 indexed referralCode);

        constructor() {
            token = IERC20(0x4E32f7eF3e364F02D762CE8519aB43273907bb2F);
            BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
            busdPair = IUniswapV2Pair(0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16);
            tokenPair = IUniswapV2Pair(0x6D3Bf7D8E6aDD090d1351A807Dcdc38BCfeE7139);
            refereeRate = 100;
            minimumStakedTokenForReferee = 100;
            //poolInfo.push(PoolInfo(300,0,259200,0,0,2500,0,0,false));//3
            //poolInfo.push(PoolInfo(1400,0,1209600,0,0,2500,0,0,true));//14
            //poolInfo.push(PoolInfo(3000,0,2592000,0,0,2500,0,0,true));//30

            poolInfo.push(PoolInfo(800,0,14 days,0,0,2500,true,100,0));//3
            poolInfo.push(PoolInfo(2000,0,30 days,0,0,2500,true,100,0));//14 
            poolInfo.push(PoolInfo(4700,0,60 days,0,0,2500,true,100,0));//30
        ...
        function addPool(PoolInfo memory pool) external onlyOwner{
            poolInfo.push(pool);
            emit AddPool(poolInfo.length-1, pool.interest, pool.lockPeriod, pool.emergencyWithdrawFee, pool.depositFee, pool.withdrawFee);
        }

        function setFees(uint256 _poolId, uint emFee, uint depFee, uint withFee) external onlyOwner {
            ...
            emit ChangedFee(_poolId, depFee, withFee, emFee);
        }

        function SetShareReferee(uint8 rate) external onlyOwner{
            require(rate <= 100, "rate should be <= 100");
            refereeRate = rate;
        }

        function setReferee(bytes32 _code,address _address) external onlyOwner{
            require(refereeAddress[_code] != _address,"Referee already set");
            refereeAddress[_code] = _address;
            addressReferee[_address] = _code;
        }
        ...
        function setMinimumStakedTokenForReferee(uint256 _minimumStakedTokenForReferee) external onlyOwner{
            require(_minimumStakedTokenForReferee !=  minimumStakedTokenForReferee,"Minimum staked token for referee already set");
            minimumStakedTokenForReferee = _minimumStakedTokenForReferee;
        }
        ...
        function getTotalUserLiveStakeAmount(address _user) public view returns(uint256) {
            StakeInfo[] storage stake = stakeInfo[_user];
            uint256 liveTotalStakedAmount = 0;
            for(uint256 i = 0; i < stake.length; ++i)
                liveTotalStakedAmount += stake[i].amount;
            return liveTotalStakedAmount;
        }
        ...
        function stakeLenght(address _user) external view returns (uint256) {
            return stakeInfo[_user].length;
        }

        function tokenAmountForBusd() public view returns(uint256){
            ...
            if(address(tokenPair.token0()) == address(token))
                (Token, BNB,) = tokenPair.getReserves();
            else
                (BNB, Token,) = tokenPair.getReserves();

            uint256 aTokenWorthOfBNB = (BNB * (10 ** 18)) / Token;

            uint256 aDollarWorthOfToken = (aDollarWorthOfBNB * (10 ** 18))/ aTokenWorthOfBNB;

            return (aDollarWorthOfToken * minimumStakedTokenForReferee);
        }

        //=======Deposit&Withdraw=======//
        function deposit(uint256 _poolId,uint256 _amount, bytes32 _refereeCode) external nonReentrant returns(bytes32){
            ...
            if(pool.depositFee>0){
                ...
                if(refereeAddress[_refereeCode]!=address(0) && refereeRate>0 && refereeAddress[_refereeCode]!=msg.sender){
                    uint256 refFee = (fee * refereeRate) / 100;
                    token.safeTransfer(refereeAddress[_refereeCode], refFee);
                    fee-=refFee;
                }
                emit DepositFee(msg.sender, fee);
            ...
            if(getTotalUserLiveStakeAmount(msg.sender) >= tokenAmountForBusd() && refereeAddress[refereeBytes32]==address(0)){
                refereeBytes32 = keccak256(abi.encodePacked(block.timestamp,msg.sender,_amount));
                refereeAddress[refereeBytes32]=msg.sender;
                addressReferee[msg.sender]=refereeBytes32;
                emit GenerateReferral(msg.sender, refereeBytes32); 
            }
