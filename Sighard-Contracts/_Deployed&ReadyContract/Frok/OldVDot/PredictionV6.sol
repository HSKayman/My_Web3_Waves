...
contract PredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor( ) {
        ...
        oracle = AggregatorV3Interface(0x435e8f18E10d7E81A7B83f638b31E6aC31cf2D5d);
        operatorAddress = msg.sender;
        intervalSeconds = 200;
        bufferSeconds = 40;
        oracleUpdateAllowance = 200;
        treasuryFee = 1;
        addressOfToken = 0xE2320dD375C634AA06c6d06725233F9837afc043;
        currentEpoch=5;
    }
//---------------------Betting Functions---------------------\\
        ...
        )
    {
        ...
        uint256 state=0;
        if(currentEpoch==5 && !paused()){
            state=7;//only Later
        }else if(currentEpoch==6 && !paused()){
            state=4;//only genesisStartRound
        }else if(currentEpoch==7&& !paused()){
            state=3;//only genesisLockRound
        }else if(currentEpoch==8&& !paused()){
            state=2;
        }else if(currentEpoch>8&& !paused()){
            state=1;
        }else if(currentEpoch>8 && paused()){
            ...
            state=7;//only genesisStartRound
        }else if(currentEpoch==8&& !paused()){
            state=6;//only genesisLockRound
        }else if(currentEpoch>8 && !paused()){
            state=5;
            ...
            state=4;
        }else if(genesisStartOnce && genesisLockOnce && !paused()){
            state=5;
        }
...
//  genesisLockOnce = false;      
    ...
    function _getPriceFromOracle() internal view returns (uint80, int256) {
        ...
        if(oracleLatestRoundId%2==uint64(0)){
            price=5e18;
        }else{
            price=10e18;
        }
