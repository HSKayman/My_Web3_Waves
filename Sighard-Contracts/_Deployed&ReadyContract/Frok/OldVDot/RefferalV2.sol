...
contract FarmarokRefferal is Ownable {
    ...
    constructor () {   
        ...
        withdrawEpoch=1669852800;
        epoch=0;
        uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(router.factory()).getPair(address(frokToken), address(router.WETH())));
        bonusRate = 50;
        bonusActive = true;
        vestingNumber = 5;
    ...
    receive() external payable {}
  	...
  	function startOrSet(bool isActive,uint256 _bonusRate,uint256 _bonusLimitperUser) public onlyOwner {
        uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(router.factory()).getPair(address(frokToken), address(router.WETH())));
    ...
    function withdrawableBonus(address _holder) public view returns(uint256){
        ...
        if(bonusClaim[_epoch-1][_holder].bonusAmount>0){
            ...
            while((DateTime.addDays(_withdrawEpoch, count*7 )) < rightNow){
    ...
    function pendingBonus(address _holder) public view returns(uint256[] memory){
        ...
        _withdrawEpoch=_withdrawEpoch= DateTime.addMonths(_withdrawEpoch, 1);
        uint256[] memory information = new uint256[](vestingNumber*2);
        information[1]=_withdrawEpoch;
    ...
    function withdrawBonusOwner(address _holder) external onlyOwner{
       ...
       if(bonusClaim[epoch-1][_holder].bonusAmount>0){
            ...
            while((DateTime.addDays(withdrawEpoch, count*7 ))< rightNow){
                sum+= (bonusClaim[epoch-1][_holder].bonusAmount+bonusClaim[epoch-1][_holder].claimedBonus) / vestingNumber;
                count+=1;
            }
            bonusClaim[epoch-1][_holder].claimedBonus+=sum;
            bonusClaim[epoch-1][_holder].bonusAmount-=sum;
            bonusClaim[epoch-1][_holder].numberOfWithdraw=count;
        }
        
        bool success=IERC20(frokToken).transfer(_holder, sum);
        if(!success) {
            revert();
        }
    }
    ...
    function withdrawBonus() external {
        ...
        for(uint256 i=0;i<epoch-1;i++){
            if(bonusClaim[i][msg.sender].bonusAmount>0){
                sum+= bonusClaim[i][msg.sender].bonusAmount;  
       ...
       if(bonusClaim[epoch-1][msg.sender].bonusAmount>0){
            ...
            while((DateTime.addDays(withdrawEpoch, count*7 ))< rightNow){
                sum+= (bonusClaim[epoch-1][msg.sender].bonusAmount+bonusClaim[epoch-1][msg.sender].claimedBonus) / vestingNumber;
                count+=1;
            }
            bonusClaim[epoch-1][msg.sender].claimedBonus+=sum;
            bonusClaim[epoch-1][msg.sender].bonusAmount-=sum;
            bonusClaim[epoch-1][msg.sender].numberOfWithdraw=count;
        }
    ...
    function viewWithdrawEpoch() public view returns(uint256,uint256){
        ...
        while((DateTime.addMonths(_withdrawEpoch, 1)) <= block.timestamp){
    ...
    function getSlippage(uint256 slippageRate,uint256 amount) public view returns (uint256) {
        if (slippageRate >= 100 || address(uniswapV2Pair) == address(0)) {
            return 0;
        }
        uint256 bnbInFrokPair;
        uint256 frokInFrokPair;
        
        if(address(uniswapV2Pair.token0()) == address(router.WETH()))
            (bnbInFrokPair, frokInFrokPair,  ) = uniswapV2Pair.getReserves();
        else
            (frokInFrokPair, bnbInFrokPair, ) = uniswapV2Pair.getReserves();
            
        uint256 aForkWorthOfBNB = (frokInFrokPair * amount* (10**18)) / bnbInFrokPair;

        return (100 - slippageRate) * aForkWorthOfBNB/(10**20);
    }
