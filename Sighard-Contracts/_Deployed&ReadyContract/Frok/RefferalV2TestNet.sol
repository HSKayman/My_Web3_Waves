...
contract FarmarokRefferal is Ownable {
    ...
    constructor () {   
        ...
        frokToken=0xE2320dD375C634AA06c6d06725233F9837afc043;
    ...
    receive() external payable {}
  	...
  	function startOrSet(bool isActive,uint256 _bonusRate,uint256 _bonusLimitperUser, uint256 _limitToken) public onlyOwner {
    ...
    function buyToken(string memory refferreCode, uint256 slippageRate) external payable {
        ...
        if(refferalCodes[encodedRefferreCode]!=address(0x0) && refferals[refferalCodes[encodedRefferreCode]].referrer!=address(msg.sender)){
            ...
            if(bonusActive){
                ...
                if(initialBalance < bonus){
                    bonus = initialBalance;
                }
    ...
    function withdrawableBonus(address _holder) public view returns(uint256){
        ...
        if(bonusClaim[_epoch-1][_holder].bonusAmount>0){
            ...
            while(count<vestingNumber){
                if((DateTime.addMinutes(_withdrawEpoch, count)) < rightNow){
    ...
    function pendingBonus(address _holder) public view returns(uint256[] memory){
        ...
        information[1]=(DateTime.addMinutes(_withdrawEpoch, 5 ));
        uint256 index=0;
        if(_epoch!=0){
            if(bonusClaim[_epoch-1][_holder].bonusAmount>0){
                uint256 count=bonusClaim[_epoch-1][_holder].numberOfWithdraw;
                while(count < vestingNumber){
                    if((DateTime.addMinutes(_withdrawEpoch, count ))> rightNow){
                        information[index*2+1] = (DateTime.addMinutes(_withdrawEpoch, count ));
        ...
        _withdrawEpoch=(DateTime.addMinutes(_withdrawEpoch, 5 ));
    ...
    function withdrawBonusOwner(address _holder) external onlyOwner{
        ...
        if(bonusClaim[epoch-1][_holder].bonusAmount>0){
            ...
            uint256 index=count;
            while(count<vestingNumber){
                if((DateTime.addMinutes(withdrawEpoch, count ))< rightNow){
        ...
        bool success=IERC20(frokToken).transfer(msg.sender, sum);
        if(!success) {
            revert();
        }
    }
    ...
    function updateWithdrawEpoch() public {
        while((DateTime.addMinutes(withdrawEpoch, 5)) <= block.timestamp){
            withdrawEpoch= DateTime.addMinutes(withdrawEpoch, 5);
            epoch++;
        }
    }
    ...
    function viewWithdrawEpoch() public view returns(uint256,uint256){
        ...
        while((DateTime.addMinutes(_withdrawEpoch, 5)) <= block.timestamp){
            _withdrawEpoch= DateTime.addMinutes(_withdrawEpoch, 5);
            _epoch++;
        }
    ...
    function setRefferalCode(string memory refferalCode) public {
        ...
        require(holderBalance >= limitToken, "You need to hold 100 FROK to set refferal code");
