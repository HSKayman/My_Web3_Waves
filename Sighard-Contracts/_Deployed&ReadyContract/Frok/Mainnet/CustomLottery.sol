    ...
    function pickWinner() external onlyOperator {
        ...
        if (currentLottery.soldTicket == 0) {
            ...
            if(currentLotteryNumber == 0){
                n_lottery[currentLotteryNumber].drawDate = startEpoch + 1 weeks;
            }else{
                n_lottery[currentLotteryNumber].drawDate = n_lottery[currentLotteryNumber-1].drawDate  + 1 weeks;
            }

            while(n_lottery[currentLotteryNumber].drawDate  + 1 weeks < block.timestamp){
                currentLotteryNumber += 1;
                n_lottery[currentLotteryNumber].drawDate = n_lottery[currentLotteryNumber-1].drawDate + 1 weeks;
                n_lottery[currentLotteryNumber].isEnded = true;
                emit LotteryWinner(currentLotteryNumber, address(0));
                n_lottery[currentLotteryNumber].winnerTicket = 0;
