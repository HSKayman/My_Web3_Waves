...
contract DividendDistributor is IDividendDistributor {
    ...
    address public RWRDToken = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    IBEP20 public RWRD = IBEP20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
...
contract TEST is IBEP20, Auth {
    ...
    uint256 private _turdFee         = 1;
    uint256 private _totalFee        = _liquidityFee + _reflectionFee + _marketingFee + _turdFee;
    uint256 public _feeDenominator   = 100;
   

    uint256 private marketingAmount     =0;
    uint256 private liquidityAmount    =0;
    uint256 private reflectionAmount   =0;
    uint256 private turdAmount         =0;
    ...
    function setFeesOnSell() private {
        ...
        _reflectionFee = sreflectionFee;
        _marketingFee = smarketingFee;
        _turdFee = sturdFee;
        _totalFee = sliquidityFee + sreflectionFee + smarketingFee + sturdFee;
    ...
    function setFeesOnBuy() private {
        ...
        _turdFee = turdFee;
        _totalFee = liquidityFee + reflectionFee + marketingFee + turdFee;
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        uint256 feeAmount = amount.mul(_totalFee).div(_feeDenominator * 100);//sil

        marketingAmount    += amount.mul(_marketingFee).div(_feeDenominator);
        liquidityAmount    +=amount.mul(_liquidityFee).div(_feeDenominator);
        reflectionAmount   +=amount.mul(_reflectionFee).div(_feeDenominator);
        turdAmount         +=amount.mul(_turdFee).div(_feeDenominator);
    ...
    function swapBack() internal swapping {
        uint256 totalAmount = marketingAmount+ liquidityAmount + reflectionAmount + turdAmount;
        uint256 dynamicLiquidityAmount= isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityAmount;

        
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityAmount).div(totalAmount).div(2);
        ...
        uint256 totalBNBAmount = totalAmount.sub(dynamicLiquidityAmount.div(2));
   

        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityAmount).div(totalBNBAmount).div(2);
        uint256 amountBNBReflection = amountBNB.mul(reflectionAmount).div(totalBNBAmount);
        uint256 amountBNBMarketing = amountBNB.mul(marketingAmount).div(totalBNBAmount);
        uint256 amountBNBTurd = amountBNB.mul(turdAmount).div(totalBNBAmount);
        ...
        marketingAmount-=swapThreshold.mul(marketingAmount).div(totalAmount);
        liquidityAmount-=swapThreshold.mul(dynamicLiquidityAmount).div(totalAmount);
        reflectionAmount-=swapThreshold.mul(reflectionAmount).div(totalAmount);
        turdAmount-=swapThreshold.mul(turdAmount).div(totalAmount);
    }
    ...
    function setFeesForBuy(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _turdFee, uint256 _feeDenominator) external authorized {
     ...
     function setFeesForSell(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _turdFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < sfeeDenominator/3, "Fees cannot be more than 33%");
    }
