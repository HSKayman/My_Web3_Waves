...
contract GizmoStaking is Ownable, ReentrancyGuard {
    ...
    constructor() {
        ...
        marketingWallet=address(0x820a27FD5C07A6c56C7819872bC6506bD0dB73a9);
        token = IERC20(0x2Bfc4De390642ccC611195E7B4a1DBD1d88d7978);
...
//=======Pool=======//
    ...
    function poolStart(uint256 _pid) external onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
        pool.startEpoch=block.timestamp;
        pool.lastRewardEpoch=block.timestamp;
        pool.rewardEndEpoch= block.timestamp + 60 days;
        pool.isOpen = true;
    }

    function poolEnd(uint256 _pid) external onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
        pool.isOpen = false;
    }
