...
contract LunaProtocol is ERC20, Ownable {
    ...
    uint256 public totalFeeonBuy = liquidityFeeonBuy + treasuryFeeonBuy + rewardFeeonBuy + insuranceFeeonBuy + burnFeeonBuy;
    uint256 public totalFeeonSell = liquidityFeeonSell + treasuryFeeonSell + rewardFeeonSell + insuranceFeeonSell + burnFeeonSell;
    ...
    event UpdateBuyFees(uint256 liquidityFeeonBuy, uint256 treasuryFeeonBuy, uint256 rewardFeeonBuy, uint256 insuranceFeeonBuy, uint256 burnFeeonBuy);
    event UpdateSellFees(uint256 liquidityFeeonSell, uint256 treasuryFeeonSell, uint256 rewardFeeonSell, uint256 insuranceFeeonSell, uint256 burnFeeonSell);
    ...
    {          
        ...
        totalFeeonBuy = liquidityFeeonBuy + treasuryFeeonBuy + rewardFeeonBuy + insuranceFeeonBuy + burnFeeonBuy;
        totalFeeonSell = liquidityFeeonSell + treasuryFeeonSell + rewardFeeonSell + insuranceFeeonSell + burnFeeonSell;

        treasuryWallet = 0xB51552B08694858db147D426945D6446B13492C1;
        insuranceWallet  = 0x7296F111256fEbB44ca6B63E86257B4A894D9156;     
    ...
    function takeFee(address sender, uint256 rAmount) internal returns (uint256) {
        ...
        uint256 burnAmount = (rAmount * _burnFee)/ 100;
    ...
    function updateBuyFees(uint256 _liquidityFeeonBuy, uint256 _treasuryFeeonBuy,uint256 _insuranceFeeonBuy, uint256 _rewardFeeonBuy, uint256 _burnFeeonBuy) external onlyOwner{
        ...
        emit UpdateBuyFees(_liquidityFeeonBuy, _treasuryFeeonBuy, _insuranceFeeonBuy, _rewardFeeonBuy, _burnFeeonBuy);
    }

    function updateSellFees(uint256 _liquidityFeeonSell, uint256 _treasuryFeeonSell,uint256 _insuranceFeeonSell, uint256 _rewardFeeonSell, uint256 _burnFeeonSell) external onlyOwner{
        ...
        require(totalFeeonBuy + totalFeeonSell <= 25, "Total fees can't be more than 25%");
        emit UpdateSellFees(_liquidityFeeonSell, _treasuryFeeonSell, _insuranceFeeonSell, _rewardFeeonSell, _burnFeeonSell);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalFee = liquidityShare + treasuryShare + insuranceShare + rewardShare;
        ...
        if(contractTokenBalance > 0){
            ...
            uint256 bnbShare = treasuryShare + insuranceShare + rewardShare;
