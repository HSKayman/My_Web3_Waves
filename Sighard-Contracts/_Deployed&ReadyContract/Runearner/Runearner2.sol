...
contract gariptooken is ERC20, Ownable {
    ...
    uint256 public rewardFee2onBuy;
    uint256 public rewardFee2onSell;
    ...
    address public immutable rewardToken = 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814; 
    address public immutable rewardTokenII = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    constructor(address newOwner) ERC20("RunEarner", "RunEarn") 
    {          
        ...
        insuranceFeeonBuy = 40;
        insuranceFeeonSell = 40;

        burnFeeonBuy = 5;
        burnFeeonSell = 5;

        totalFeeonBuy = liquidityFeeonBuy + treasuryFeeonBuy + rewardFeeonBuy + rewardFee2onBuy + insuranceFeeonBuy + burnFeeonBuy;
        totalFeeonSell = liquidityFeeonSell + treasuryFeeonSell + rewardFeeonSell + rewardFee2onSell + insuranceFeeonSell + burnFeeonSell;

        treasuryWallet = 0xCd4B94b8b3DAB0EcacC140b13730dC7C8b66a2D6;
        insuranceWallet  = 0x3D8bb0Cec54eC75c7e5e064e2dE7600E2B67435b;     

        walletToWalletTransferWithoutFee = true;
        dividendTracker = new DividendTracker(40000, rewardToken);
        dividendTracker2 = new DividendTracker(40000, address(0));
        ...
        dividendTracker2.excludeFromDividends(address(dividendTracker));
        dividendTracker2.excludeFromDividends(address(this));
        dividendTracker2.excludeFromDividends(DEAD);
        dividendTracker2.excludeFromDividends(address(_uniswapV2Router));
    ...
    function _transferFrom(
        ...
        try dividendTracker2.setBalance(payable(sender), balanceOf(sender)) {} catch {}
        try dividendTracker2.setBalance(payable(recipient), balanceOf(recipient)) {} catch {}
        if(!inSwap) {
	    	uint256 gas = gasForProcessing / 2;
    ...
    function takeFee(address sender, uint256 rAmount) internal returns (uint256) {
        ...
        if(uniswapV2Pair == sender){
            _finalFee = totalFeeonBuy;
            _burnFee = burnFeeonBuy;
        } else{
            _finalFee = totalFeeonSell;
        ...
        _rBalance[address(this)] = _rBalance[address(this)] + (feeAmount-burnAmount);
    ...
    function updateBuyFees(uint256 _liquidityFeeonBuy, uint256 _treasuryFeeonBuy,uint256 _insuranceFeeonBuy, uint256 _rewardFeeonBuy, uint256 _rewardFee2onBuy, uint256 _burnFeeonBuy) external onlyOwner{
        liquidityFeeonBuy = _liquidityFeeonBuy;
        treasuryFeeonBuy = _treasuryFeeonBuy;
        insuranceFeeonBuy = _insuranceFeeonBuy;
        rewardFeeonBuy = _rewardFeeonBuy;
        rewardFee2onBuy = _rewardFee2onBuy;
        burnFeeonBuy = _burnFeeonBuy;
        totalFeeonBuy = _liquidityFeeonBuy + _treasuryFeeonBuy + _insuranceFeeonBuy + _rewardFeeonBuy + _burnFeeonBuy;
        require(totalFeeonBuy + totalFeeonSell <= 25, "Total fees can't be more than 25%");
        emit UpdateBuyFees(_liquidityFeeonBuy, _treasuryFeeonBuy, _insuranceFeeonBuy, _rewardFeeonBuy, _rewardFee2onBuy, _burnFeeonBuy);
    }

    function updateSellFees(uint256 _liquidityFeeonSell, uint256 _treasuryFeeonSell,uint256 _insuranceFeeonSell, uint256 _rewardFeeonSell, uint256 _rewardFee2onSell, uint256 _burnFeeonSell) external onlyOwner{
        liquidityFeeonSell = _liquidityFeeonSell;
        treasuryFeeonSell = _treasuryFeeonSell;
        insuranceFeeonSell = _insuranceFeeonSell;
        rewardFeeonSell = _rewardFeeonSell;
        rewardFee2onSell = _rewardFee2onSell;
        burnFeeonSell = _burnFeeonSell;
        totalFeeonSell = _liquidityFeeonSell + _treasuryFeeonSell + _insuranceFeeonSell + _rewardFeeonSell + _burnFeeonSell;
        require(totalFeeonBuy + totalFeeonSell <= 25, "Total fees can't be more than 25%");
        emit UpdateSellFees(_liquidityFeeonSell, _treasuryFeeonSell, _insuranceFeeonSell, _rewardFeeonSell, _rewardFee2onSell, _burnFeeonSell);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 reward2Share = rewardFee2onBuy + rewardFee2onSell;
        uint256 totalFee = liquidityShare + treasuryShare + insuranceShare + rewardShare + reward2Share;
        ...
        if(contractTokenBalance > 0){
            ...
            uint256 bnbShare = treasuryShare + insuranceShare + rewardShare + reward2Share;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(rewardShare > 0) {
                    ...
                    swapAndSendDividends(rewardBNB,true);
                }
                if(reward2Share > 0) {
                    uint256 reward2BNB = (newBalance * reward2Share) / bnbShare;
                    swapAndSendDividends(reward2BNB,false);
                }
            }
        }
    }
    ...
    function swapAndSendDividends(uint256 amount, bool isOne) private{
        if(isOne){    
            address[] memory path = new address[](2);
    ...
    //=======Divivdend Tracker=======//
    function updateDividendTracker(address newAddress,bool isOne) public onlyOwner {
        if(isOne){
            ...
            dividendTracker2 = newDividendTracker;
        }
    }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance, bool isOne) external onlyOwner {
        if(isOne){
    ...
    function updateClaimWait(uint256 claimWait, bool isOne) external onlyOwner {
        if(isOne){
    ...
    function getClaimWait(bool isOne) external view returns(uint256) {
        if(isOne){
            return dividendTracker.claimWait();
        }else{
            return dividendTracker2.claimWait();
        }
    }

    function getTotalDividendsDistributed(bool isOne) external view returns (uint256) {
        if(isOne){
    ...
    function withdrawableDividendOf(address account,bool isOne) public view returns(uint256) {
        if(isOne){
	...
	function dividendTokenBalanceOf(address account,bool isOne) public view returns (uint256) {
        if(isOne){
    ...
    function totalRewardsEarned(address account,bool isOne) public view returns (uint256) {
        if(isOne){
            return dividendTracker.accumulativeDividendOf(account);
        }else{
            return dividendTracker2.accumulativeDividendOf(account);
        }
    }
    
	function excludeFromDividends(address account,bool isOne) external onlyOwner{
        if(isOne){
    ...
    function getAccountDividendsInfo(address account,bool isOne)
	...
	function getAccountDividendsInfoAtIndex(uint256 index,bool isOne)
        external view returns (
            address,
            int256,
            int256,
        ...
        if(isOne){
            ...
            return dividendTracker2.getAccountAtIndex(index);
        }
    }

	function processDividendTracker(uint256 gas, bool isOne) external {
        if(isOne){
            (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = dividendTracker.process(gas);
            emit ProcessedDividendTracker(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
        }else{
            (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = dividendTracker2.process(gas);
            emit ProcessedDividendTracker(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
        }
    }

    function claim(bool isOne) external {
        if(isOne){
    ...
    function claimAddress(address claimee, bool isOne) external onlyOwner {
        if(isOne){
            dividendTracker.processAccount(payable(claimee), false);
        }else{
            dividendTracker2.processAccount(payable(claimee), false);
        }
    }

    function getLastProcessedIndex(bool isOne) external view returns(uint256) {
        if(isOne){
            return dividendTracker.getLastProcessedIndex();
        }else{
            return dividendTracker2.getLastProcessedIndex();
        }
    }

    function setLastProcessedIndex(uint256 index, bool isOne) external onlyOwner {
        if(isOne){
            dividendTracker.setLastProcessedIndex(index);
        }else{
            dividendTracker2.setLastProcessedIndex(index);
        }
    }

    function getNumberOfDividendTokenHolders(bool isOne) external view returns(uint256) {
        if(isOne){
